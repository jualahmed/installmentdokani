...
class Account_model extends CI_model{
	...
	function all_today_transaction_sale()
	{
		...
		$this->db->select('transaction_info.transaction_id,transaction_info.transaction_purpose,transaction_info.amount,transaction_info.date,transaction_info.remarks,customer_info.customer_name');
		$this->db->from('transaction_info,customer_info');
		$this->db->where('transaction_info.ledger_id=customer_info.customer_id AND (transaction_info.transaction_purpose="sale" OR transaction_info.transaction_purpose="interestsale" OR transaction_info.transaction_purpose="installmentfee")');
		if($start_date!=''){$this -> db -> where('transaction_info.date >= "'.$start_date.'"');}
		if($end_date!=''){$this -> db -> where('transaction_info.date <= "'.$end_date.'"');}
		else if($start_date!=''){$this -> db -> where('transaction_info.date <= "'.$start_date.'"');}
		$query = $this->db->get();
	...
	function all_today_transaction_income()
	{
		$start_date=$this->input->post('start_date');
		$end_date=$this->input->post('end_date');
		$this->db->select('transaction_info.transaction_id,transaction_info.transaction_purpose,transaction_info.amount,transaction_info.date,transaction_info.remarks,type_info.type_type,income_info.income_details');
		$this->db->from('transaction_info,income_info,type_info');
		$this->db->where('transaction_info.common_id=income_info.income_id');
		$this->db->where('transaction_info.sub_id=type_info.type_id');
		$this->db->where('transaction_info.transaction_purpose="income"');
		if($start_date!=''){$this -> db -> where('transaction_info.date >= "'.$start_date.'"');}
		if($end_date!=''){$this -> db -> where('transaction_info.date <= "'.$end_date.'"');}
		else if($start_date!=''){$this -> db -> where('transaction_info.date <= "'.$start_date.'"');}
		$query = $this->db->get();
	...
	function all_today_transaction_expense_get()
	{
		$start_date=$this->input->post('start_date');
		$end_date=$this->input->post('end_date');
		$this->db->select('transaction_info.transaction_id,transaction_info.transaction_purpose,transaction_info.amount,transaction_info.date,transaction_info.remarks,type_info.type_type,income_info.income_details');
		$this->db->from('transaction_info,type_info');
		$this->db->join('income_info','transaction_info.common_id=income_info.income_id','left');
		$this->db->where('transaction_info.sub_id=type_info.type_id');
		$this->db->where('transaction_info.transaction_purpose="income_get"');
		if($start_date!=''){$this -> db -> where('transaction_info.date >= "'.$start_date.'"');}
		if($end_date!=''){$this -> db -> where('transaction_info.date <= "'.$end_date.'"');}
		else if($start_date!=''){$this -> db -> where('transaction_info.date <= "'.$start_date.'"');}
		$query = $this->db->get();
	...
	*****************************/
	...
	{
		$this->db->select('SUM(transaction_info.amount) as total_sale_amount,customer_info.customer_id,customer_info.customer_name,customer_info.customer_contact_no');
		$this->db->from('transaction_info,customer_info');			
	...
	public function redundancy_check($table, $field1, $field2, $element1, $element2)
	{
	...
	public function loan_person_info()
	{
	...
	public function all_sale($start,$end,$customer_id)
	{
		...
		$this->db->group_by('transaction_info.transaction_id');
		$query_data = $this->db->get();
		if($query_data->num_rows() > 0)return $query_data;
		else return false;
	}
	...
	public function transaction_with_employees($specific,$employee_id)
	{
		if($specific)
		{
